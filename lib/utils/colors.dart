import 'package:flutter/material.dart';

// ================= Widget Use Colors // Compulsory ======================

const colorPrimary = Colors.teal;
const textColorPrimary = Color(0xFF252525);
const textGreenColor = Color(0xFF00c97b);
const textColorSecondary = Color.fromARGB(255, 37, 133, 244);
const color_white = Color(0xFFFFFFFF);
const view_color = Color(0xFFB4BBC2);
const Dissmisable_RedColor = Color(0xffE41414);
const textcolorSecondary = Color(0xff8391A1);

// ================ Primray // Compulsory//  ===============================

const iconColorPrimary = Color(0xFFFFFFFF);
const iconColorSecondary = Color(0xFFA8ABAD);
const appSecondaryBackgroundColor = Color(0xFF131d25);
const appTextColorPrimary = Color(0xFF212121);
const appTextColorSecondary = Color(0xFF5A5C5E);
const appShadowColor = Color(0x95E9EBF0);
const appColorPrimaryLight = Color(0xFFF9FAFF);

// ======================  Dark Theme Colors // Compulsory// =================

const appBackgroundColorDark = Color(0xFF121212);
const cardBackgroundBlackDark = Color(0xFF1F1F1F);
const color_primary_black = Color(0xFF131d25);
const appColorPrimaryDarkLight = Color(0xFFF9FAFF);
const iconColorPrimaryDark = Color(0xFF212121);
const iconColorSecondaryDark = Color(0xFFA8ABAD);
const appShadowColorDark = Color(0x1A3E3942);
const appColorPrimary = Color(0xFF1157FA);
const appLayout_background = Color(0xFFf8f8f8);

// ======================  Apps Colors // Compulsory// =================
const textGreyColor = Color(0xff747688);
const filedColor = Color(0xfff5f5f5);
const liver_color = Color(0xFFAC0D40);
const ContainerBoder = Color(0xffE8ECF4);
const pinkColor = Color(0xffB34167);
const iconGrey_color = Color(0xFFD5D7DC);
const grocery_colorPrimary_light = Color(0xFFE7FAEE);
const grocery_textColorPrimary = Color(0xFF120D26);

const grocery_textGreenColor = Color(0xFF00c97b);

const grocery_edittext_background = Color(0xFFF7F8F9);
const grocery_Color_black = Color(0xFF000000);
const grocery_color_white = Color(0xFFFFFFFF);

const grocery_app_background = Color(0xFFE7EFF6);
const grocery_color_yellow = Color(0xFFFFC107);
const grocery_orangeColor = Color(0xFFFE7858);
const grocery_orangeColor_light = Color(0xFFfff0b3);
const grocery_purpleLightColor = Color(0xFF7c5cf4);
// const grocery_ShadowColor = Color(0X95E9EBF0);
const grocery_view_color = Color(0xFFB4BBC2);
const grocery_orangeLight_Color = Color(0xFFFAA421);
const grocery_RedLight_Color = Color(0xFFFA4063);
const grocery_Red_Color = Color(0xFFFA244D);

const grocery_light_gray_color = Color(0xFFD0D3D9);
const grocery_DarkBlue_Color = Color(0xFF2233FF);
const grocery_blue_Color = Color(0xFF4A00FF);
const grocery_Pink_Color = Color(0xFFFE2986);

const grocery_blue = Color(0xFF6897BB);
const grocery_darkGrey = Color(0xFF949494);
const grocery_lightGrey = Color(0xFFc9c9c9);
const dating_orange = Color(0xFFFFA500);
